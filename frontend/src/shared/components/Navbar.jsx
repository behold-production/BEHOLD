...
export default function Navbar({ navigateToSection, currentView, onOpenAuth, siteName, siteSettings }) {
  ...
  return (
    <>
          ...
          <div className={useTransparentHeader
            ...
            <div className="flex items-center gap-3 sm:gap-5 shrink-0">
              ...
              <button
                ...
                className={useTransparentHeader ? "hidden" : "hidden lg:flex items-center justify-center px-6 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer border-none hover:scale-105 active:scale-95 shadow-md bg-neon-blue-deep text-white hover:bg-neon-blue-mid shadow-neon-blue-deep/10"}
              >
                Book Appointment
              </button>
      ...
      <div
        id="mobile-menu"
        className={`fixed top-0 left-0 h-full w-[85vw] max-w-[340px] z-50 shadow-2xl lg:hidden flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
        style={{ background: 'linear-gradient(160deg, #152b52 0%, #0c1a36 60%, #060e20 100%)' }}
      >
        {/* ── Header ── */}
        <div className="px-6 pt-6 pb-5 flex items-center justify-between border-b border-white/8">
          <button
            onClick={logoClick}
            className="inline-flex items-baseline bg-transparent border-none p-0 cursor-pointer text-white font-header font-black text-2xl tracking-wider uppercase drop-shadow-sm"
            style={{ fontWeight: 900, fontFamily: 'Outfit, sans-serif', lineHeight: 1 }}
          >
            <span style={{ fontWeight: 900 }}>{siteName || 'BEHOLD'}</span>
            <span className="text-[#00E5FF] font-black ml-0.5" style={{ fontWeight: 900, color: '#00E5FF' }}>.</span>
        ...
        <div className="flex-1 overflow-y-auto px-3 pt-4 pb-2">
          {[
            ...
            >
              <span>{label}</span>
              <svg className="w-4 h-4 text-white/25 group-hover:text-gold-soft group-hover:translate-x-0.5 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {socials.map((link, idx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-9 h-9 rounded-full border border-white/12 flex items-center justify-center text-white/50 hover:text-gold-soft hover:border-gold-soft/40 transition-all duration-200"
                  >
                    {renderSocialIcon(link.name, link.logo)}
                  </a>
                ))}
        ...
        <div className="px-5 pb-6 pt-4 border-t border-white/8 flex flex-col gap-3">
          ...
          {siteSettings?.contactPhone && (
            ...
            >
              <svg className="w-4 h-4 text-gold-soft shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 h-14 flex items-center justify-center rounded-[2px] font-semibold text-sm text-green-deep border-none cursor-pointer transition-all active:scale-95 bg-gold hover:bg-gold-soft shadow-md shadow-gold/10"
            >
              Book Appointment
            </button>
          </div>
