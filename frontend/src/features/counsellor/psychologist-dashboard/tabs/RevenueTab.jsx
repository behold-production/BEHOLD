...
export default function RevenueTab(props) {
 ...
 <div className="bg-zinc-900/40 border border-zinc-800 p-5 rounded-[10px] space-y-2 shadow-lg">
 <div className="flex justify-between items-center text-zinc-500">
 <span className="text-xs font-bold tracking-wider">Pending Payouts</span>
 <AlertCircle className="w-4 h-4 text-amber-500" />
 </div>
 <div className="text-2xl font-bold text-amber-550 font-header">
 ₹{formatAmount(metrics.pendingPayouts)}
 </div>
 ...
 {/* Bank Routing details */}
  ...
  <span className="text-[10px] bg-zinc-800 border border-zinc-700 text-zinc-400 font-bold px-2 py-0.5 rounded">
  Bank Info Pending
  </span>
  )}
  </div>
  ...
  <div className="p-3 bg-zinc-950/60 border border-zinc-800 rounded-[10px] text-[11px] text-zinc-400 font-medium leading-relaxed">
  💳 <strong>Direct Settlement Info:</strong> Admin processes periodic payout transfers directly into your registered bank account based on your settled console earnings.
  </div>
  </div>
 ...
 <td className="p-3 text-right font-semibold text-zinc-450">₹{formatAmount(gross)}</td>
 <td className="p-3 text-right font-bold text-emerald-450">₹{formatAmount(myPayout)}</td>
