...
export default function CdatSection({ setView, siteSettings }) {
  ...
  return (
    <section id="cdat" className="py-20 sm:py-28 bg-transparent text-surface-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div>
              <span className="text-xs font-semibold tracking-widest uppercase text-[#00e5ff] flex items-center gap-1.5 mb-2">

                {sectionSub}
              </span>
              <h2 id="aptitude-title" className="text-3xl sm:text-5xl font-sans font-semibold uppercase text-[#0f172a] tracking-tight leading-[1.05]">
            ...
            <p className="text-xs font-semibold uppercase tracking-widest text-surface-500">
              Assessment Partner &mdash; <span className="text-[#0f172a] font-semibold">CIGI</span>
            </p>
            ...
            <div className="space-y-4 pt-2">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="font-semibold text-[#0f172a] text-sm uppercase mb-1">{step.label}</div>
          ...
          <div className="lg:col-span-5 bg-white rounded-xl p-6 sm:p-10 shadow-md border border-surface-200">
            <h3 className="text-xl sm:text-2xl font-sans font-semibold uppercase text-[#0f172a] mb-2">
              Register for the C-DAT<span className="text-[#00e5ff] drop-shadow-[0_0_8px_rgba(0,229,255,0.8)] font-semibold">.</span>
            </h3>
            ...
            {!generatedCode ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-[#0f172a] uppercase mb-1.5">Full Name *</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#0f172a] uppercase mb-1.5">Phone Number *</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#0f172a] uppercase mb-1.5">Email Address *</label>
                  <input
                ...
                <button
                  type="button"
                  onClick={() => { window.spaNavigate?.('/sample-test'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                  className="w-full py-3 bg-surface-100 hover:bg-surface-200 text-[#0f172a] font-semibold text-xs uppercase tracking-widest rounded-full transition-all border border-surface-200 cursor-pointer"
                >
              ...
              <div className="space-y-4 py-2">
                <div className="bg-surface-50 border border-surface-200 rounded-xl p-6 text-center">
                  <p className="text-xs font-semibold uppercase tracking-widest text-surface-500 mb-2">Your Group Code</p>
                  <p className="text-2xl font-sans font-semibold text-[#0f172a] tracking-wider select-all">{generatedCode}</p>
                </div>
                <button
                  onClick={copyCode}
