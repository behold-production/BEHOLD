...
const DEFAULT_APTITUDE_QUESTIONS = [
  {
    id: 'q1',
    category: 'Logical',
...
export default function AptitudeTest({ onFinishTest }) {
  ...
  useEffect(() => {
    async function loadQuestions() {
      try {
        ...
        const questions = Array.isArray(res?.data) && res.data.length > 0 ? res.data : DEFAULT_APTITUDE_QUESTIONS;
        setDbQuestions(questions);
        const shuffled = [...questions].sort(() => 0.5 - Math.random());
        setShuffledQuestions(shuffled);
      } catch (error) {
        console.error("Failed to load aptitude questions, using defaults:", error);
        setDbQuestions(DEFAULT_APTITUDE_QUESTIONS);
        const shuffled = [...DEFAULT_APTITUDE_QUESTIONS].sort(() => 0.5 - Math.random());
        setShuffledQuestions(shuffled);
      } finally {
        setIsLoading(false);
      }
 ...
 return (
  <div 
      ...
      >
 <p className="text-[10px] font-semibold tracking-widest text-surface-500 animate-pulse">Initializing quiz console...</p>
 </div>
 );
 }
 ...
 <span className="text-surface-900 font-semibold text-sm sm:text-lg self-start sm:self-auto">
 ...
 <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-surface-100 text-surface-900 font-semibold text-sm shrink-0 mt-0.5 ">
 Q
 </span>
 <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-surface-900 leading-snug">
 ...
 <div>
 <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-[10px] bg-surface-50 border border-surface-200 text-surface-900 text-[10px] tracking-widest font-semibold mb-1">
 ...
 <h3 className="text-xs font-semibold text-surface-500 tracking-wider uppercase">
 ...
 <div>
 <h4 className="text-base sm:text-lg font-semibold text-surface-900 tracking-wider leading-tight mb-2 uppercase">
 {dominantInfo.title}
 </h4>
 ...
 <h4 className="font-semibold text-surface-900 text-xs tracking-wider uppercase border-b border-surface-200 pb-3">
 ...
 <span className="text-xs font-semibold text-surface-900 tracking-wider">
 {item.career}
 </span>
 </div>
 ...
 <p className="text-[10px] font-semibold tracking-widest text-surface-500 text-center mt-3 flex items-center justify-center gap-1">
 ...
 <h4 className="font-semibold text-surface-900 text-[10px] tracking-widest">
 ...
 <div className="flex justify-between text-[10px] font-semibold text-surface-500 tracking-widest">
 <span>{label}</span>
