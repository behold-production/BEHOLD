          ...
          <div className="bg-white border border-surface-200 rounded-[10px] shadow-square-light p-5">
            ...
            {!testProfile ? (
              <div className="bg-surface-50 border border-dashed border-surface-300 rounded-[10px] p-8 text-center">
                ...
                <p className="text-sm font-black capitalize tracking-widest text-surface-900">No Sample Test History</p>
                ...
                <button
                  ...
                  className="mt-4 inline-flex items-center gap-1.5 px-5 py-2 bg-surface-900 text-white capitalize tracking-widest text-[10px] font-black rounded-[10px] hover:bg-surface-800 transition-colors border-none shadow-none"
                >
              ...
              <div className="space-y-4">
                ...
                <div className="bg-surface-900 rounded-[10px] p-5 text-white">
                  <p className="text-[10px] text-surface-400 font-bold capitalize tracking-widest mb-1">Dominant Domain</p>
                  <h4 className="text-xl font-black capitalize tracking-widest text-white">{topDomain}</h4>
                  <p className="text-xs text-surface-400 mt-1.5">
                    Your primary strength outcome. Click below to view the full detailed breakdown.
                  </p>
                  <div className="mt-3.5 flex justify-between items-center">
                    <span className="text-[10px] capitalize tracking-widest font-black px-2.5 py-1 bg-white/10 rounded-[10px]">
                ...
                <div className="space-y-3 pt-2">
                  <p className="text-[10px] font-bold text-surface-500 capitalize tracking-widest">Top Strength Domains</p>
                  {scores.slice(0, 4).map(([key, pct]) => (
                    <div key={key} className="space-y-1">
                      <div className="flex justify-between capitalize tracking-widest font-black text-[10px]">
                ...
                <button
                  type="button"
                  onClick={() => navigate('/sample-test')}
                  className="w-full mt-2 inline-flex items-center justify-center gap-1.5 px-4 py-2 border border-surface-200 hover:border-surface-300 text-surface-900 capitalize tracking-widest text-[10px] font-black rounded-[10px] hover:bg-surface-50 transition-colors bg-white cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {CAREER_SUGGESTIONS[topDomain].map((career, i) => (
                  <div key={i} className="bg-surface-50 border border-surface-200 rounded-[10px] p-2.5 flex items-center gap-2">
                    ...
                    <span className="text-[10px] capitalize tracking-widest font-bold text-surface-900">{career}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="xl:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-surface-200 rounded-[10px] shadow-square-light p-5">
            ...
            <form onSubmit={handleCigiUpload} className="space-y-3.5">
              <div>
                <label className="text-[10px] capitalize tracking-widest font-bold text-surface-600 block mb-1">
                ...
                <input
                  ...
                  required
                  className="w-full text-sm font-medium text-surface-900 file:mr-3 file:py-1.5 file:px-3 file:rounded-[10px] file:border-0 file:text-[10px] file:capitalize file:tracking-widest file:font-black file:bg-surface-200 file:text-surface-900 hover:file:bg-surface-300 file:cursor-pointer p-2 border border-surface-200 rounded-[10px] bg-surface-50 focus:outline-none"
                />
                <p className="text-[10px] font-bold capitalize tracking-widest text-surface-400 mt-1">Allowed formats: JPG, JPEG, PNG, PDF (Max 5MB)</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] capitalize tracking-widest font-bold text-surface-600 block mb-1">Date Taken</label>
                  <input
                    type="date"
                    value={cigiDate}
                ...
                <div>
                  <label className="text-[10px] capitalize tracking-widest font-bold text-surface-600 block mb-1">Time Taken</label>
                  <input
                    type="time"
                    value={cigiTime}
              ...
              <div>
                <label className="text-[10px] capitalize tracking-widest font-bold text-surface-600 block mb-1">Remarks / Note</label>
                <textarea
              ...
              <button
                ...
                className="w-full min-h-[38px] inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-surface-900 hover:bg-surface-800 disabled:bg-surface-500 text-white rounded-full capitalize tracking-widest text-[10px] font-black transition-colors border-none shadow-none cursor-pointer"
              >
                {isCigiUploading ? (
          ...
          <div className="bg-white border border-surface-200 rounded-[10px] shadow-square-light p-5">
            ...
            {cigiResultsList.length === 0 ? (
              <div className="text-center py-6 text-surface-400 border border-dashed border-surface-200 rounded-[10px] bg-surface-50">
                <p className="text-[10px] font-bold capitalize tracking-widest">No result files uploaded yet.</p>
              </div>
            ) : (
                  ...
                  <div key={result.id} className="p-3 bg-white border border-surface-200 rounded-[10px] flex items-center justify-between gap-3 text-left">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-[10px] font-black capitalize ${result.fileType === 'pdf' ? 'bg-rose-100 text-rose-700' : 'bg-blue-100 text-blue-700'}`}>
                          {result.fileType}
                        </span>
                      ...
                      <p className="text-[10px] text-surface-400 font-bold capitalize tracking-widest mt-0.5">
