        ...
        <div className={getGridClasses()}>
          {visibleServices.map((service, idx) => {
            ...
            return (
              <div
                key={service.id}
                className={`group relative bg-white/95 backdrop-blur-md rounded-3xl ${isSingle ? 'p-8 sm:p-10' : 'p-6 sm:p-7'} border border-slate-200/90 shadow-md hover-scale-card hover:border-[#00c9d6]/60 transition-all duration-300 flex flex-col justify-between overflow-hidden reveal-on-scroll reveal-scale-in ${delayClass}`}
              >
                <div>
                  {/* Top Bar: Icon */}
                  <div className={`${isSingle ? 'w-14 h-14' : 'w-11 h-11'} rounded-2xl bg-[#00c9d6]/10 text-[#007078] flex items-center justify-center mb-5 group-hover:bg-[#00c9d6] group-hover:text-slate-950 group-hover:scale-110 transition-all duration-300 shadow-xs`}>
