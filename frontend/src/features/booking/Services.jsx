...
function ReadMore({ text, color = 'text-[#00A8FF]' }) {
...
export default function Services({ setView, onBookTherapist, siteSettings, mode }) {
  ...
  const [activeIntroCard, setActiveIntroCard] = useState(() => {
    if (!enableCareerMentoring && enablePsychology) return 'counsel';
    return 'career';
  });

  useEffect(() => {
    if (!enableCareerMentoring && enablePsychology && activeIntroCard === 'career') {
      setActiveIntroCard('counsel');
    } else if (!enablePsychology && enableCareerMentoring && activeIntroCard === 'counsel') {
      setActiveIntroCard('career');
    }
  }, [enableCareerMentoring, enablePsychology, activeIntroCard]);
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
              ...
              <div className="lg:col-span-7">
                ...
                <div className="flex gap-2 mb-4 bg-gray-100/80 p-1.5 rounded-lg w-fit border border-gray-200/50">
                  {enableCareerMentoring && (
                    <button
                      onClick={() => setActiveIntroCard('career')}
                      className={`px-4.5 py-2 rounded-lg text-xs sm:text-sm font-bold transition border-none cursor-pointer flex items-center gap-2 ${activeIntroCard === 'career' ? 'bg-[#00F0FF]/10 text-[#00A8FF] border border-[#00F0FF]/30 shadow-sm' : 'bg-transparent text-gray-600 hover:text-gray-900'}`}
                    >
                      <span>🎯</span>
                  ...
                  {enablePsychology && (
                    <button
                      ...
                      className={`px-4.5 py-2 rounded-lg text-xs sm:text-sm font-bold transition border-none cursor-pointer flex items-center gap-2 ${activeIntroCard === 'counsel' ? 'bg-white text-pink-600 shadow-sm' : 'bg-transparent text-gray-600 hover:text-gray-900'}`}
                    >
                      <span>🧠</span>
                ...
                <div className="relative min-h-[380px]">
                  {((activeIntroCard === 'career' || !enablePsychology) && enableCareerMentoring) && (
                    <div className="bg-white rounded-lg p-6 sm:p-8 border border-gray-100 shadow-xl transition-all duration-500 ease-out flex flex-col justify-between h-full">
                      <div>
                        <div className="flex items-center justify-between mb-5">
                          <div className="flex items-center gap-3">
                            <div className="w-11 h-11 bg-[#00F0FF]/10 border border-[#00F0FF]/30 rounded-lg flex items-center justify-center shrink-0">
                              <svg className="w-5 h-5 text-[#00A8FF]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                <circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" />
                                <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
                              </svg>
                            </div>
                            <div>
                              <span className="text-xs font-bold uppercase tracking-wider text-[#00A8FF] block">{settings.careerBadge || 'Career Mentoring'}</span>
                              <span className="text-[11px] text-gray-400 font-medium">Lifetime Access & Roadmaps</span>
                            </div>
                          </div>
                          <span className="px-2.5 py-1 bg-[#00F0FF]/10 border border-[#00F0FF]/30 text-[#00A8FF] font-bold text-[11px] rounded-full">Slide 1 / 2</span>
                        </div>
                        ...
                        <ReadMore color="text-[#00A8FF]" text={settings.careerDesc || "Whether you're choosing a stream, exploring career options, or planning your future studies, we help you understand your strengths, interests, and opportunities so you can make confident decisions."} />
                        <ul className="space-y-2 mt-4 mb-6">
                          {['Stream & Degree Selection Guidance', 'Strengths & Aptitude Mapping', 'Personalized Career Roadmap'].map(item => (
                            <li key={item} className="flex items-center gap-2 text-gray-700 text-xs sm:text-sm font-medium">
                              <svg className="w-4 h-4 text-[#00F0FF] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
                              {item}
                            </li>
                          ))}
                        </ul>
                      ...
                      <button
                        ...
                        className="w-full py-3 bg-[#00F0FF] hover:bg-[#00d8e6] text-white font-bold rounded-lg transition border-none cursor-pointer text-sm shadow-md shadow-[#00F0FF]/20 flex items-center justify-center"
                      >
                  ...
                  {((activeIntroCard === 'counsel' || !enableCareerMentoring) && enablePsychology) && (
                    <div className="bg-white rounded-lg p-6 sm:p-8 border border-gray-100 shadow-xl transition-all duration-500 ease-out flex flex-col justify-between h-full">
                      <div>
                        <div className="flex items-center justify-between mb-5">
                          <div className="flex items-center gap-3">
                            <div className="w-11 h-11 bg-pink-100 rounded-lg flex items-center justify-center shrink-0">
                      ...
                      <button
                        ...
                        className="w-full py-3 bg-pink-600 hover:bg-pink-700 text-white font-bold rounded-lg transition border-none cursor-pointer text-sm shadow-md shadow-pink-500/10 flex items-center justify-center"
                      >
                  ...
                  <div className="flex justify-center gap-2 mt-4">
                    {enableCareerMentoring && (
                      <button
                        onClick={() => setActiveIntroCard('career')}
                        className={`w-2 h-2 rounded-full transition-all border-none cursor-pointer ${activeIntroCard === 'career' ? 'bg-[#00F0FF] w-6' : 'bg-gray-300 hover:bg-gray-400'}`}
                      />
                    )}
                    {enablePsychology && (
                      <button
              ...
              <div className="lg:col-span-5 flex flex-col justify-center py-4">
                <div id="unfold-behold" className="mb-3">
                  <span className="text-xs font-bold tracking-widest uppercase text-[#00A8FF]">Unfold with Behold</span>
                </div>
                ...
                <div className="space-y-3.5 mb-8">
                  <div className="flex items-start gap-3 bg-white p-3.5 rounded-lg border border-gray-100 shadow-sm">
                    <div className="w-8 h-8 rounded-lg bg-[#00F0FF]/10 flex items-center justify-center text-[#00A8FF] shrink-0 text-base font-bold">✨</div>
                    <div>
                ...
                <div>
                  <button
                    ...
                    className="px-6 py-3 bg-gray-900 hover:bg-[#00A8FF] text-white font-bold rounded-lg transition border-none cursor-pointer shadow-md text-sm inline-flex items-center justify-center"
                  >
        ...
        {(!mode || mode === 'experts') && (
          <div className={mode === 'experts' ? '' : 'mt-20'}>
            <div className="text-center mb-14">
              <span className="text-xs sm:text-sm font-bold tracking-widest uppercase text-[#00A8FF] block mb-3">Our Clinical Team</span>
              <h2 id="experts-title" className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 mb-4 tracking-tight leading-tight flex items-center justify-center flex-wrap">
                {renderTitleWithFullstopDot('Meet Our Experts', 'about-title', 'Scroll to What We Offer ↓', 'md')}
              </h2>
              <p className="text-base sm:text-lg text-gray-600 font-normal max-w-2xl mx-auto leading-relaxed">Certified professionals dedicated to your wellbeing and career success.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8 bg-gray-50 p-4 rounded-lg border border-gray-200">
              <div className="flex gap-2 flex-wrap">
                {['All', 'Consultant Psychologist', 'Clinical Psychologist', 'Career Mentor'].map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    className={`px-4 py-2 rounded-lg font-semibold text-sm transition border-none cursor-pointer ${filter === f ? 'bg-[#00A8FF] text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200'}`}>
                    {f === 'All' ? 'All Roles' : f}
                  </button>
                ))}
              </div>
              <div className="flex-1 md:max-w-xs ml-auto">
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#00A8FF] text-sm"
                />
              </div>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <button onClick={() => { setFilter('All'); setSearch(''); }} className="mt-4 px-6 py-2 bg-[#00A8FF] text-white rounded-lg font-semibold border-none cursor-pointer">Reset Filters</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.slice(0, 6).map(advisor => (
                  <div key={advisor.id} className="bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    {/* Top */}
                    <div className="p-6 pb-4">
                      <div className="flex items-center gap-4 mb-4">
                        <div className="w-14 h-14 rounded-lg bg-[#00F0FF]/10 text-[#00A8FF] flex items-center justify-center font-black text-xl overflow-hidden shrink-0">
                          {advisor.profilePic
                        ...
                        <div>
                          ...
                          <p className="text-[#00A8FF] text-sm font-semibold">{advisor.role}</p>
                        </div>
                      </div>
                    ...
                    <div className="p-4 flex gap-3 border-t border-gray-100">
                      <button
                        ...
                        className="flex-1 py-2.5 border-2 border-gray-200 hover:border-[#00F0FF] text-gray-700 hover:text-gray-950 font-bold rounded-lg text-sm transition bg-transparent cursor-pointer"
                      >
                        View Profile
                      </button>
                      <button
                        onClick={() => { if (onBookTherapist) onBookTherapist(advisor.id); else window.spaNavigate('/booking'); window.scrollTo({ top: 0 }); }}
                        className="flex-1 py-2.5 bg-[#00F0FF] hover:bg-[#00d8e6] text-white font-black rounded-lg text-sm transition border-none cursor-pointer shadow-[0_0_12px_rgba(0,240,255,0.3)]"
                      >
                        Book Now
                      </button>
                    </div>
