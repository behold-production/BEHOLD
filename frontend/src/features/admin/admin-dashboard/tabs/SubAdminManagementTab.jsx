...
export default function SubAdminManagementTab(props) {
 ...
 className="text-rose-500 hover:text-rose-455 font-bold text-xs cursor-pointer border-none bg-transparent"
 >
 Delete
 </button>
 )}
 {isProtected && (
 <span className="text-[10px] text-zinc-550 border border-zinc-800 bg-zinc-950 px-1 py-0.2 rounded font-bold">System</span>
 )}
 </div>
 </div>
 {role.description ? (
 ...
 <span className="text-[10px] text-zinc-500 font-bold block">Modules ({enabledModules.length})</span>
 <div className="flex flex-wrap gap-1">
 {enabledModules.length > 0 ? (
 enabledModules.map(m => (
 <span key={m.id} className="px-1.5 py-0.5 rounded bg-zinc-955 text-[10px] text-zinc-400 border border-zinc-850">
 ...
 <label className="text-zinc-400 font-bold text-sm ">Staff Name</label>
 <input
 type="text"
 required
 ...
 <div className="space-y-1">
 <label className="text-zinc-400 font-bold text-sm ">Role Title</label>
 <select
 ...
 <label className="text-zinc-400 font-bold text-sm ">Email Address</label>
 <input
 type="email"
 required
 ...
 <div className="sm:col-span-2 space-y-1">
 <label className="text-zinc-400 font-bold text-sm ">Password</label>
 <input
 type="password"
 required
 placeholder="••••••••"
 ...
 {subAdminError && (
 <p className="text-sm text-rose-500 font-bold ">{subAdminError}</p>
 )}

 {subAdminSuccess && (
 <p className="text-sm text-emerald-500 font-bold ">{subAdminSuccess}</p>
 )}
 ...
 <p className="text-sm text-zinc-450 font-bold ">Inherited Scopes for {subAdminForm.roleName}</p>
 ...
 <div className="flex flex-wrap gap-1">
 {enabledModules.map(m => (
 <span key={m.id} className="px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 text-xs font-bold text-zinc-400 ">
 {m.name.replace(' Management', '').replace(' & Leads', '')}
 </span>
 ))}
 {enabledModules.length === 0 && (
 ...
 <div className="space-y-1">
 <label className="text-zinc-400 font-bold text-sm">Role Title Name</label>
 <input
 type="text"
 required
 ...
 <div className="space-y-1">
 <label className="text-zinc-400 font-bold text-sm">Role Description</label>
 <input
 type="text"
 ...
 <label className="text-zinc-400 font-bold text-sm block">All Privileges & Scope Toggles</label>
 ...
 {roleError && (
 <p className="text-sm text-rose-500 font-bold ">{roleError}</p>
 )}
 {roleSuccess && (
 <p className="text-sm text-emerald-500 font-bold ">{roleSuccess}</p>
 )}
