...
export default function SectionHeader({ subtitle, title, description }) {
    return (
        <div className="flex flex-col space-y-3 mb-10 w-full max-w-3xl mx-auto items-center text-center">
            {subtitle && (
                <span className="text-[#00E5FF] font-semibold text-xs tracking-[0.2em] uppercase">
                    {subtitle}
                </span>
            )}
            <h2 className="text-3xl md:text-4xl lg:text-[42px] font-semibold text-slate-950 tracking-tight leading-tight font-header">
                {title}
            </h2>
            {description && (
