...
export default function App() {
  ...
  const [activeDocType, setActiveDocType] = useState(null); // 'terms', 'privacy', 'refund', 'consent'
  ...
  const handleDownloadPDF = async () => { //
    try {
      ...
      const content = activeDocType === 'terms' ? (siteSettings.termsOfUse || defaultTermsOfUse) : activeDocType === 'refund' ? (siteSettings.refundPolicy || defaultRefundPolicy) : activeDocType === 'consent' ? (siteSettings.consentPolicy || defaultConsentPolicy) : (siteSettings.privacyPolicy || defaultPrivacyPolicy);
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-950/40">
              <h3 className="text-sm font-semibold text-slate-900 font-header flex items-center gap-2">
              ...
              <button
                ...
                className="p-1 bg-zinc-955 border border-zinc-850 hover:border-zinc-700 text-zinc-400 hover:text-slate-900 rounded-lg transition-all cursor-pointer border-none shadow-sm flex items-center justify-center"
                title="Close modal"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-6 overflow-y-auto text-left text-zinc-300 text-xs font-semibold leading-relaxed whitespace-pre-wrap font-sans max-h-[60vh] custom-scrollbar">
              {activeDocType === 'terms'
                ? (siteSettings.termsOfUse || defaultTermsOfUse)
                : activeDocType === 'refund'
                ? (siteSettings.refundPolicy || defaultRefundPolicy)
                : activeDocType === 'consent'
                ? (siteSettings.consentPolicy || defaultConsentPolicy)
                : (siteSettings.privacyPolicy || defaultPrivacyPolicy)}
            </div>
            ...
            <div className="px-6 py-4 border-t border-zinc-800 flex justify-end gap-3 bg-zinc-955">
              <button
                ...
                className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-slate-900 font-semibold text-xs rounded-xl cursor-pointer transition border border-zinc-700 shadow-md flex items-center gap-2"
              >
              ...
              <button
                onClick={() => setActiveDocType(null)}
                className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-750 text-slate-900 hover:text-brand font-semibold text-xs rounded-lg cursor-pointer transition border-none shadow-md"
              >
                Close Document
              </button>
            </div>
      ...
      {siteSettings?.whatsapp && !hideNavbarAndFooter && (() => {
        ...
        return (
          <a
            ...
            className={`fixed ${isProfilePage ? 'bottom-20 lg:bottom-6' : 'bottom-6 sm:bottom-6'} right-4 sm:right-6 z-50 bg-[#25D366] hover:bg-[#20ba5a] text-slate-900 p-3.5 sm:p-4 rounded-full shadow-[0_4px_16px_rgba(37,211,102,0.45)] hover:shadow-[0_6px_24px_rgba(37,211,102,0.6)] hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center cursor-pointer group`}
            title="Chat with us on WhatsApp"
            aria-label="Chat with us on WhatsApp"
          >
            <span className="absolute -top-10 right-0 bg-zinc-900 text-slate-900 text-[11px] font-semibold px-2.5 py-1 rounded-md shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap border border-zinc-800">
              Chat on WhatsApp
            </span>
            <svg className="w-7 h-7 sm:w-8 sm:h-8 fill-current text-slate-900" viewBox="0 0 24 24">
